...
def add_figures(doc):
    figure_files = [
        ("degradation_curves.png", "NASA battery degradation curves (SOH vs. cycle) for the four classic cells (B0005, B0006, B0007, B0018). B0006 crosses the EOL threshold at approximately cycle 130."),
        ("auc_by_horizon.png", "Model discrimination by prediction horizon on NASA 4-cell data. Raw XGBoost AUC is near-random across all horizons; calibrated AUC is flat at 0.50 because isotonic regression preserves rank order and the model produces near-constant predictions."),
        ("dataset_composition.png", "Dataset composition showing total discharge cycles and EOL cycles (SOH < 0.70) per NASA classic cell."),
        ("feature_correlation.png", "Feature correlation matrix for the NASA dataset. SOH and derived features show moderate correlation with voltage and temperature."),
        ("ablation.png", "Ablation study comparing failure rates across dispatch configurations on NASA 4-cell data. The 4-cell dataset provides insufficient signal for dispatch differentiation."),
        ("real_vs_synthetic.png", "Comparison of real NASA degradation (left) and synthetic degradation (right). The generator produces qualitatively similar trajectories but with approximately 1.7x faster mean degradation (KS test D=0.33, p<0.001)."),
        ("scaling_curve.png", "Figure 1: Synthetic scaling study — macro-averaged AUC vs number of batteries (N). Shaded region shows 95% bootstrap confidence intervals (20 Monte Carlo seeds per N). Regime annotations: insufficient (N ≤ 5), marginal (5 < N < 12), reliable (N ≥ 12). The curve rises from AUC 0.84 (N=2) to 0.97 (N=8) and plateaus above 0.98 for N ≥ 12."),
    ]
