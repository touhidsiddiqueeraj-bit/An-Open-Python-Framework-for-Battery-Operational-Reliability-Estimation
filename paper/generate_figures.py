...
def fig7_scaling_curve():
    ...
    ci_low = np.array(d["auc_ci_low"])
    ci_high = np.array(d["auc_ci_high"])
    fig, ax = plt.subplots(figsize=(8, 5))
    ax.plot(N, auc_mean, "-o", color="#1b3a5c", linewidth=2, markersize=6, zorder=3)
    ax.fill_between(N, ci_low, ci_high,
                    alpha=0.2, color="#1b3a5c", label="95% bootstrap CI")
    ...
    ax.axvspan(12, 22, alpha=0.06, color="#27ae60", label="Reliable (N≥12)")
